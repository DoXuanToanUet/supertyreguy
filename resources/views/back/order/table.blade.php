...
@php
    ...
    if ($data->additional_cost){
      foreach(json_decode($data->additional_cost,true) as $key => $value){
      $aditional_value+=$value;
    }
    }
@endphp
<tr id="order-bulk-delete">
  <td><input type="checkbox" class="bulk-item" value="{{$data->id}}"></td>
    ...
    <td>
      @if ($setting->currency_direction == 1)
      {{$data->currency_sign}}{{PriceHelper::OrderTotalGST($data)}}
      @else
      {{PriceHelper::OrderTotalGST($data)}}{{$data->currency_sign}}
      @endif
    </td>

    <td>
    @if ($setting->currency_direction == 1)
      {{$data->currency_sign}}{{$aditional_value}}
      @else
      {{$aditional_value}}{{$data->currency_sign}}
      @endif
    </td>
