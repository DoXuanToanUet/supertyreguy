...
class CheckoutController extends Controller
{
    ...
    public function __construct(FrontRepository $repository)
    {
        ...
        $this->service_repository = $repository;
    }
	...
	public function ship_address()
	{
        ...
        $data['cart'] = $cart;
        $data['cart_total'] = $cart_total+($cart_total*.15)+$aditional_cost;
        $data['grand_total'] = $total_amount+($total_amount*.15)+$aditional_cost;
    ...
    public function shipping()
    {
        ...
        $total_amount = $grand_total;
        $data['cart'] = $cart;
        $data['cart_total'] = $cart_total+$aditional_cost;
        $data['grand_total'] = $total_amount+$aditional_cost;
        $data['discount'] = $discount;
        $data['shipping'] = $shipping;
        $data['tax'] = $total_tax;
        $data['payments'] = PaymentSetting::whereStatus(1)->get();
    ...
    {   
        ...
        $data['cart'] = $cart;
        $data['cart_total'] = $cart_total+($cart_total*.15)+$aditional_cost;
        $data['grand_total'] = $total_amount+($cart_total*.15)+$aditional_cost;
        $data['discount'] = $discount;
        $data['shipping'] = $shipping;
        $data['tax'] = $total_tax;
        $data['payments'] = PaymentSetting::whereStatus(1)->get();
    ...
    public function orderPlace(){
        $data =$this->service_repository->getAllServices();
        return view('front.book_order.booking', ['data' => $data]);
    }

    public function orderBookingPost(Request $request){
        $clientList =$this->service_repository->insertClient();
        if($clientList['status']){
            $result=$this->service_repository->productBooking($request->all(),$clientList['email'],$clientList['name']);
            if($result['status']){
                try{
                    // $to_email = "sho105328@gmail.com";
                    Mail::to($result['email'])->send(new BookingOrder($result));
                    if(Mail::failures() != 0) {
                        return redirect()->route('front.index')->withSuccess('Check your mail for booking details');
                    }
                    else {
                        return redirect()->route('front.index')->withSuccess('Successfully booking done.');
                    }
                }catch(Exception $e){
                    // dd($e->getMessage());
                    return redirect()->route('front.index')->withError('Something went wrong.');
                }
                // return redirect()->route('front.index')->withSuccess('Successfully booking done.');
            }
        }
        return redirect()->route('front.book_order.booking')->withError('Booking not done');
    }
