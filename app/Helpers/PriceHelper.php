...
class PriceHelper
{
    ...
    // GST Calculate
    public static function setCurrencyGSTPrice($price) {
        ...
        $setting = Setting::first();
        $result=$price*$curr->value +($price*$curr->value*.15);
        if($setting->currency_direction == 1){
         return $curr->sign . round($result,2);
        }else{
         return  round($result,2).$curr->sign;
        }
    }
    ...
    public static function OrderTotalGST($order)
    {
        $cart = json_decode($order->cart,true);
        ...
        $result_amount=$grand_total+$grand_total*.15;
        $total_amount = round($result_amount * $order->currency_value,2);
